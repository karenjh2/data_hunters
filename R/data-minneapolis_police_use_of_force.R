...
#' 
#' From Minneapolis, data from 2008 thru August 2021
#'
#' @format A data frame with 34115 rows and 15 variables.
#' \describe{
...
#'   \item{latitude}{Latitude where response occurred.}
#'   \item{longitude}{Longitude where response occurred.}
#' }
#' @examples
#' library(dplyr)
#' library(ggplot2)
...
#' minneapolis_police_use_of_force %>%
#'   count (race) %>% 
...
#' ggplot(minneapolis_police_use_of_force %>% filter(race %in% race_sub),
